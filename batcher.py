...
class Example(object):
    ...
    def __init__(self, article, abstract_sentences, vocab, hps, file_id):
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list, hps):
        ...
        self.enc_lens:
          numpy array of shape (batch_size) containing integers. The (truncated) length of each encoder input sequence
          (pre-padding).
        self.enc_padding_mask:
          numpy array of shape (batch_size, <=max_enc_steps), containing 1s and 0s. 1s correspond to real tokens in
          enc_batch and target_batch; 0s correspond to padding.
    ...
    """
        ...
        # Note: our enc_batch can have different length (second dimension) for each batch because we
        # use dynamic_rnn for the encoder.
...
class Batcher(object):
    ...
    def __init__(self, data_path, vocab, hps, single_pass, cluster):
    ...
    Args:
      ...
      vocab: Vocabulary object
      hps: hyperparameters
      single_pass: If True, run through the dataset exactly once (useful for when you want to run evaluation on the dev
       or test set). Otherwise generate random batches indefinitely (useful for training).
    """
        ...
        if single_pass:
            ...
            # this essentially means no bucketing
    ...
    def fill_example_queue(self):
        ...
        if self._cluster:
            input_gen = self.text_gen(data.eg_gen(self._data_path))
        else:
            input_gen = self.text_generator(data.example_generator(self._data_path, self._single_pass))

        while True:
            try:
                (article, abstract, file_id) = input_gen.next()
            ...
            # print(abstract, '\n==========:::::::===')
            # print(data.abstract2sents(abstract))
            # raise ValueError()
            abstract_sentences = [sent.strip() for sent in abstract]
            # Use the <s> and </s> tags in abstract to get a list of sentences.
            example = Example(article, abstract_sentences, self._vocab, self._hps, file_id)  # Process into an Example.
    ...
    def text_gen(self, eg_gen):
        ...
        while True:
            ...
            if True:
                ...
                # print(split)
                filepath = '/Data/anubhavcs17/asurl/cluster_abs/{}/{}_{}/{}/{}.json'.format('use', 'use', 'kmed', split,
                                                                                            file_id)
                if split == 'train':
                    num_clusters = len(abs)
                clustered_articles = ["" for _ in range(num_clusters)]
                try:
                    cluster_res = json.load(open(filepath))
                except:
                    print(filepath, len(art), len(abs))
                    continue
                cluster_res = cluster_res['labels']
                assert len(cluster_res) == len(art), "{} {} {}\n{}".format(filepath, len(cluster_res), len(art),
                                                                           art)
                ...
                for idx, cluster in enumerate(clustered_articles):
                    if split == 'train':
                        ...
                        if len(abs[idx]) < 2:
                            ...
                            print(abs)
                            raise ValueError
                        yield (cluster, [abs[idx]], file_id)
                    else:
                        yield (cluster, abs, file_id)
    ...
    Args:
      example_generator: a generator of tf.Examples from file. See data.example_generator"""
        while True:
            e, f, c = example_generator.next()  # e is a tf.Example
            try:
                ...
                split = Path(f).stem.split('_')[0]
                file_id = c - 1 + int(Path(f).stem.split('_')[-1])
                ...
                with open(filepath, 'r') as f:
                    ...
                    assert len(cluster_res) == len(article_sents), "{} {} {}\n{}".format(filepath, len(cluster_res),
                                                                                         len(article_sents),
                                                                                         article_sents)
                    for idx, c_id in enumerate(cluster_res):
                        clustered_articles[c_id] += article_sents[idx]
            except ValueError:
                tf.logging.error('Failed to get article or abstract from example')
                continue
